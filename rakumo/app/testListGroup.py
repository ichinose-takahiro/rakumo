...
CSVFILE = WORKDIR + '180514_GroupSession_change.csv'
...
def getProcess():
    ...
    for clData in clList:
        cnnt = cnnt + 1
        logging.debug(str(cnnt))
        clData = json.loads(clData,encoding='utf-8')
        #writeData = clData
        
        
        title = clData['SUMMARY']+clData['STARTDATE']+clData['ENDDATE']
        if clData['SCD_GRP_SID'] != '-1':
            title = title + clData['SCD_GRP_SID']

#        logging.debug(title)
        if title in checkList:
            #checkList[title]['SCD_GRP_SID'] = clData['SCD_GRP_SID']
            #checkList[title]['SUMMARY'] = clData['SUMMARY']
            #checkList[title]['STARTDATE'] = clData['STARTDATE']
            #checkList[title]['ENDDATE'] = clData['ENDDATE']
            checkList[title]['CNT'] = checkList[title]['CNT'] + 1
 #           logging.debug(str(checkList[title]['CNT']))
        else:
            checkList[title] = {}
            checkList[title]['SCD_GRP_SID'] = clData['SCD_GRP_SID']
            checkList[title]['SUMMARY'] = clData['SUMMARY']
    ...
    for k, checkData in checkList.items():
#        logging.debug(k)
#        logging.debug(checkData['CNT'])
        if int(checkData['CNT']) >= 80:
            logging.debug(checkData['SCD_GRP_SID']+','+checkData['SUMMARY']+','+checkData['STARTDATE']+','+checkData['ENDDATE']+','+str(checkData['CNT']))
