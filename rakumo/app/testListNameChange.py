...
def getProcess():
    ...
    for clData in clList:
        ...
        if priEmail != memData['pri_email']:
            ...
            pricEmail, pricnt = selectEmail(pricnt)
            logging.debug('prichange')
            logging.debug(pricEmail)

        writeData['PRISEI'] , writeData['PRIMEI'] = changeSeiMei(pricEmail)

        memEmail, memcnt = selectEmail(memcnt)
        logging.debug(memEmail)
        writeData['SEI'], writeData['MEI'] = changeSeiMei(memEmail)
